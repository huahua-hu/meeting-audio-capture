...
final class TranscriptJournalTests: XCTestCase {
    func testSortsBothTracksByStartTimeAndOnlyMergesAdjacentSpeakerTurns() async throws {
        ...
        try await journal.append(.init(speaker: .interviewer, sessionStartedAt: sessionStartedAt, startTime: 5, endTime: 6, text: "请介绍自己"))
        try await journal.append(.init(speaker: .me, sessionStartedAt: sessionStartedAt, startTime: 3, endTime: 4, text: "您好"))
        try await journal.append(.init(speaker: .interviewer, sessionStartedAt: sessionStartedAt, startTime: 1, endTime: 2, text: "你好"))
        try await journal.append(.init(speaker: .interviewer, sessionStartedAt: sessionStartedAt, startTime: 7, endTime: 8, text: "最近做了什么"))
        ...
        let firstInterviewer = try XCTUnwrap(markdown.range(of: "[2026-07-08 19:00:01] 面试官：你好"))
        let me = try XCTUnwrap(markdown.range(of: "[2026-07-08 19:00:03] 我：您好"))
        let secondInterviewer = try XCTUnwrap(
            markdown.range(of: "[2026-07-08 19:00:05] 面试官：请介绍自己最近做了什么")
        )
